...
const maxPathsInput = document.getElementById("maximum-paths-input");
...
visualizeAlgorithmBtn.onclick = function() {
    ...
    graph.resetColor();
    if(worker === null) {
        worker = new Worker("/js/algorithms/heuristicAlgorithmWorker.js");
    ...
    const maxPaths = maxPathsInput.value;
    
    worker.postMessage({ type, vertices, edges, delay, maxPaths });
    algorithmPaused = false;
    algorithmWorking = true;
    algorithmOutput.innerText = "The algorithm is currently running!";
}
...
runAlgorithmBtn.onclick = async function() {
    ...
    const algorithmConfig = {
        ...
        maxPaths: maxPathsInput.valueAsNumber
    };
    ...
    algorithmOutput.value = "The heuristic is running!\nPlease wait";
