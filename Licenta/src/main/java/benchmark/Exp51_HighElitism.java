...
public class Exp51_HighElitism {
    static final int RUNS = 5;

    public static void main(String[] args) throws Exception {
        int popSize = 2000;
        int elitism = 200; // 10% instead of 5%
        String configName = String.format("ILP+Roulette_pop%d_elite%d", popSize, elitism);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp51_HighElitism_" + timestamp + ".csv");
