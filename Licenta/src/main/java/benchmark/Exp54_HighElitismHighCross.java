...
public class Exp54_HighElitismHighCross {
    static final int RUNS = 3;

    public static void main(String[] args) throws Exception {
        double crossRate = 0.5;
        String configName = String.format("ILP+Roulette_pop2000_elite200_cross%.1f", crossRate);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp54_HighElitismHighCross_" + timestamp + ".csv");
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath)) {
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                config.setProbabilityCrossover(crossRate);
                ...
                csv.write(String.format("%s,yeast,%d,%d,%d,%d,2000,200,0.5,%.1f,ROULETTE,%d,%.3f,%.3f%n",
                        configName, V, E, r + 1, ilpSeed, crossRate, len, gaElapsed, totalElapsed));
                csv.flush();
            }
