...
/**
 * Exp32: ILP(2min) + Roulette with popSize=2000 on yeast.
 * Default popSize=min(V,1000)=1000 for yeast. Doubling may improve diversity.
 * Baseline: ILP+Roulette(p=2, pop=1000) gives yeast 258/253.6.
 */
public class Exp32_LargerPopYeast {
    public static void main(String[] args) throws Exception {
        int runs = 5;
        int ilpTimeoutSeconds = 120;

        String experimentId = "Exp32_LargePop_Yeast";
        String description = "ILP(2min) + Roulette popSize=2000 on yeast";
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath);
            ...
            for (int r = 0; r < runs; r++) {
                ...
                System.out.printf("  yeast run %d: %d in %.1fs (pop=2000, seed=%d)%n", r + 1, len, elapsed, ilpSeed);
                csv.write(String.format("yeast,%d,%d,%d,%d,2000,%d,%.3f%n", V, E, r + 1, ilpSeed, len, elapsed));
                csv.flush();
            }
            ...
            summary.write(String.format("yeast |V|=%d |E|=%d  Best=%d Avg=%.1f Worst=%d AvgTime=%.1fs ILPSeed=%d PopSize=2000 Runs=%s%n",
