...
public class Exp46_HighCrossover {
    static final int RUNS = 5;

    public static void main(String[] args) throws Exception {
        double crossoverRate = args.length > 0 ? Double.parseDouble(args[0]) : 0.6;
        String configName = String.format("ILP+Roulette_pop2000_cross%.1f", crossoverRate);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp46_HighCrossover_" + timestamp + ".csv");
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath)) {
            csv.write("Config,Instance,|V|,|E|,Run,ILPSeed,PopSize,Elitism,MutRate,CrossRate,Selection,MaxGenIncrease,PathLength,GATime(s),TotalTime(s)\n");
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                config.setProbabilityCrossover(crossoverRate);
                config.setMaxGenerationIncrease(400);
                ...
                csv.write(String.format("%s,yeast,%d,%d,%d,%d,2000,100,0.5,%.1f,ROULETTE,400,%d,%.3f,%.3f%n",
                        configName, V, E, r + 1, ilpSeed, crossoverRate, len, gaElapsed, totalElapsed));
                csv.flush();
            }
