...
public class Exp48_HighPressure {
    static final int RUNS = 5;

    public static void main(String[] args) throws Exception {
        double pressure = args.length > 0 ? Double.parseDouble(args[0]) : 3.0;
        String configName = String.format("ILP+Roulette_pop2000_pressure%.0f", pressure);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp48_HighPressure_" + timestamp + ".csv");
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath)) {
            csv.write("Config,Instance,|V|,|E|,Run,ILPSeed,PopSize,Elitism,MutRate,Pressure,Selection,PathLength,GATime(s),TotalTime(s)\n");
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                config.setSelectionPressure(pressure);
                ...
                csv.write(String.format("%s,yeast,%d,%d,%d,%d,2000,100,0.5,%.1f,ROULETTE,%d,%.3f,%.3f%n",
                        configName, V, E, r + 1, ilpSeed, pressure, len, gaElapsed, totalElapsed));
                csv.flush();
            }
