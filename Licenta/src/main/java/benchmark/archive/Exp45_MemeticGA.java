...
public class Exp45_MemeticGA {
    static final int RUNS = 5;

    public static void main(String[] args) throws Exception {
        int topK = args.length > 0 ? Integer.parseInt(args[0]) : 5;
        String configName = String.format("Memetic_top%d_pop2000_mut0.5", topK);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp45_MemeticGA_" + timestamp + ".csv");
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath)) {
            csv.write("Config,Instance,|V|,|E|,Run,ILPSeed,PopSize,Elitism,MutRate,Selection,MemeticTopK,PathLength,GATime(s),TotalTime(s)\n");
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                config.setMemeticLocalSearch(true);
                config.setMemeticTopK(topK);
                ...
                csv.write(String.format("%s,yeast,%d,%d,%d,%d,2000,100,0.5,ROULETTE,%d,%d,%.3f,%.3f%n",
                        configName, V, E, r + 1, ilpSeed, topK, len, gaElapsed, totalElapsed));
                csv.flush();
            }
