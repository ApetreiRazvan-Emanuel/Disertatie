...
public class Exp49_HigherMutation {
    static final int RUNS = 5;

    public static void main(String[] args) throws Exception {
        double mutRate = args.length > 0 ? Double.parseDouble(args[0]) : 0.7;
        String configName = String.format("ILP+Roulette_pop2000_mut%.1f", mutRate);

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp49_HigherMutation_" + timestamp + ".csv");
