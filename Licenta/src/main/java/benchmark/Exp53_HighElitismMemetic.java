...
public class Exp53_HighElitismMemetic {
    static final int RUNS = 3;

    public static void main(String[] args) throws Exception {
        String configName = "ILP+Roulette_pop2000_elite200_memetic5";

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        java.nio.file.Path outputPath = Paths.get("src/main/output/experiments/Exp53_HighElitismMemetic_" + timestamp + ".csv");
        ...
        try (BufferedWriter csv = Files.newBufferedWriter(outputPath)) {
            csv.write("Config,Instance,|V|,|E|,Run,ILPSeed,PopSize,Elitism,MutRate,Memetic,Selection,PathLength,GATime(s),TotalTime(s)\n");
            ...
            for (int r = 0; r < RUNS; r++) {
                ...
                csv.write(String.format("%s,yeast,%d,%d,%d,%d,2000,200,0.5,top5,ROULETTE,%d,%.3f,%.3f%n",
                        configName, V, E, r + 1, ilpSeed, len, gaElapsed, totalElapsed));
                csv.flush();
            }
