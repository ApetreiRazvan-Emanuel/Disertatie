...
public class GeneticAlgorithmConfig {
    ...
    private boolean useGreedyBoost;
    private double greedyBoostRatio;
    private boolean usePathCrossover;
    private int heavyKickInterval;
    private int heavyKickCount;
    private boolean diversifySeeds;
    private int perturbAllowedLoss;
    ...
    public GeneticAlgorithmConfig(Graph graph) {
        ...
        // Population size: 2000 for large graphs (Exp51 champion config)
        popSize = Math.min(Math.max(graph.numVertices(), 200), 2000);
        ...
        // 10% elitism (Exp51 champion config — sweet spot for diversity vs preservation)
        elitism = popSize / 10;
        ...
        // Roulette selection (Exp51 champion config — scales better at pop=2000 than tournament)
        selectionMode = SelectionMode.ROULETTE;
        tournamentSize = 2;
        ...
        // Memetic local search enabled — perturbation/splice/reroute operators are essential for quality
        memeticLocalSearch = false;
        memeticTopK = 5;

        // Greedy boost: use residual-neighbor heuristic in extendFrom (like ACO)
        useGreedyBoost = false;
        greedyBoostRatio = 0.7;
        usePathCrossover = false;
        heavyKickInterval = 20;
        heavyKickCount = 1;
        diversifySeeds = false;
        perturbAllowedLoss = 0;
    }
    ...
    public boolean isUseGreedyBoost() {
        return useGreedyBoost;
    }

    public void setUseGreedyBoost(boolean useGreedyBoost) {
        this.useGreedyBoost = useGreedyBoost;
    }

    public double getGreedyBoostRatio() {
        return greedyBoostRatio;
    }

    public void setGreedyBoostRatio(double greedyBoostRatio) {
        this.greedyBoostRatio = greedyBoostRatio;
    }

    public boolean isUsePathCrossover() {
        return usePathCrossover;
    }

    public void setUsePathCrossover(boolean usePathCrossover) {
        this.usePathCrossover = usePathCrossover;
    }

    public int getHeavyKickInterval() {
        return heavyKickInterval;
    }

    public void setHeavyKickInterval(int heavyKickInterval) {
        this.heavyKickInterval = heavyKickInterval;
    }

    public int getHeavyKickCount() {
        return heavyKickCount;
    }

    public void setHeavyKickCount(int heavyKickCount) {
        this.heavyKickCount = heavyKickCount;
    }

    public boolean isDiversifySeeds() {
        return diversifySeeds;
    }

    public void setDiversifySeeds(boolean diversifySeeds) {
        this.diversifySeeds = diversifySeeds;
    }

    public int getPerturbAllowedLoss() {
        return perturbAllowedLoss;
    }

    public void setPerturbAllowedLoss(int perturbAllowedLoss) {
        this.perturbAllowedLoss = perturbAllowedLoss;
    }
}
